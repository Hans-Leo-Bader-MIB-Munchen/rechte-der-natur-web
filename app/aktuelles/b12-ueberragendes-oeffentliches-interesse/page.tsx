...
export default function B12Artikel() {
  return <main className="newsPage">
    ...
    <article style={{maxWidth:860,margin:"0 auto",padding:"0 28px 72px",fontSize:"1.08rem",lineHeight:1.75}}>
      <aside style={{margin:"0 0 38px",padding:"22px 24px",background:"rgba(0,0,0,.035)",borderLeft:"4px solid var(--green)"}}>
        <p style={{margin:"0 0 8px"}}><strong>Worum geht es?</strong></p>
        <p style={{margin:0}}>Heute kann gerichtlich geprüft werden, ob beim Ausbau der B12 die geltenden Umweltvorschriften eingehalten werden. Eigene Rechte der Mitwelt würden eine zusätzliche Frage schaffen: <strong>Verletzt der Ausbau Rechte der betroffenen Mitwelt selbst?</strong> Genau dieser Unterschied ist der Kern dieses Beitrags.</p>
      </aside>
      ...
      <p>Entscheidend ist, was danach geschah: Die Ergänzung der Umweltprüfung erfolgte nicht unabhängig vom Rechtsstreit. Nach eigener Darstellung entschied die Regierung von Schwaben <strong>aus Anlass der anhängigen Gerichtsverfahren</strong>, die Umweltverträglichkeitsstudie zu den Schutzgütern globales Klima und Fläche vorsorglich ergänzen zu lassen und diese Ergänzungen in die Abwägung einzubeziehen. Dabei geht es vereinfacht um die Auswirkungen des Vorhabens auf das Klima und um die zusätzlich beanspruchte Fläche. Seit März 2024 konnten Betroffene und Verbände zu den ergänzten Unterlagen Stellung nehmen; 2025 erließ die Regierung einen Ergänzungsbeschluss und hielt nach erneuter Abwägung am geplanten Ausbau fest.</p>
      <p><strong>Damit hatten die anhängigen Gerichtsverfahren bereits vor einer gerichtlichen Entscheidung eine konkrete Wirkung:</strong> Nach eigener Darstellung der Regierung waren sie der Anlass, Teile der Umweltprüfung noch einmal aufzugreifen und vorsorglich zu ergänzen. Das bedeutet allerdings nicht, dass ein Gericht die Ergänzung angeordnet oder bereits einen Rechtsfehler festgestellt hätte. Die Regierung bezeichnete die Ergänzung selbst als vorsorglich.</p>
      ...
      <p>Aber daraus folgt nicht, dass der Verband rechtlich „die Natur“ oder ein konkretes Ökosystem repräsentiert. <strong>Eine Umweltverbandsklage kontrolliert, ob geltendes Umweltrecht eingehalten wird. Eigenrechte der Mitwelt stellen eine andere Frage: Hat die Mitwelt selbst eine Rechtsposition, die verletzt werden kann?</strong></p>
      ...
      <p>Das bedeutet jedoch nicht, dass die B12 mit Eigenrechten automatisch anders entschieden oder verhindert würde. Das „überragende öffentliche Interesse“ am Straßenbau verschwände dadurch nicht. Vielmehr müsste geklärt werden, welchen konkreten Inhalt die Rechte der Mitwelt haben, wann sie verletzt sind, wer sie vertreten und gerichtlich geltend machen kann und wie diese Rechtsposition zu entgegenstehenden gesetzlichen Interessen steht. <strong>Eigenrechte verändern damit die Rechtsfrage – ein anderes Ergebnis des B12-Verfahrens lässt sich daraus allein nicht ableiten.</strong></p>
