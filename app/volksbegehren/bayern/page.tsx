  ...
  return (
    <main className={styles.page}>
      ...
      <section className={styles.process} id="weg">
        <div className={styles.sectionHead}><p className={styles.label}>Der demokratische Weg</p><h2>Ein Volksbegehren wird irgendwann sehr konkret.</h2></div>
        <div className={styles.steps}>
          <article><span>1</span><h3>Zulassungsantrag</h3><p>Zunächst müssen genügend gültige Unterstützungsunterschriften gesammelt werden. Dafür braucht es Mitstreiter, Sammelstellen, Öffentlichkeit und Organisation.</p></article>
          <article><span>2</span><h3>Volksbegehren</h3><p>Nach der Zulassung folgt die gesetzlich geregelte Eintragungsphase. Jetzt entscheidet sich, ob aus Zustimmung tatsächlich mobilisierte Beteiligung wird.</p></article>
      ...
      <section className={styles.today} id="heute">
        <div><p className={styles.label}>Was wir heute genauer sehen</p><h2>Ein Recht braucht einen Weg in die Wirklichkeit.</h2></div>
        <div className={styles.todayText}>
          ...
          <p>Das schwächt die Kampagne nicht. Es gibt ihr eine belastbarere Grundlage.</p>
        </div>
      </section>

      <section className={styles.bridge}>
        <p className={styles.label}>Vom ersten Versuch zur bundesweiten Kampagne</p>
        <h2>Bayern bleibt der Ausgangspunkt – aber nicht die Schablone für alle anderen Länder.</h2>
        <p>Für jedes Bundesland müssen Rechtslage, Verfahren, politische Situation und mögliche Formulierungen neu geprüft werden. Wo daraus ein tragfähiger Weg entsteht, wollen wir Menschen zusammenbringen und eine reale Initiative aufbauen.</p>
      ...
      <section className={styles.bridge}>
        <p className={styles.label}>Weiter geht es nur mit Menschen</p>
        <h2>Eine Kampagne braucht Mitstreiter – und irgendwann Unterschriften.</h2>
        <p>Wenn du die Rechte der Natur in Bayern oder einem anderen Bundesland voranbringen willst, geht es nicht nur um Zustimmung. Wir brauchen Menschen, die organisieren, prüfen, kommunizieren, sammeln und Verantwortung übernehmen.</p>
        <Link href="/mitmachen">Mitmachen →</Link>
      </section>

      <footer className={styles.footer}><Link href="/volksbegehren">← Zu den Volksbegehren</Link><span>Rechte der Natur · Bayern · Preview</span></footer>
    </main>
  );
}
